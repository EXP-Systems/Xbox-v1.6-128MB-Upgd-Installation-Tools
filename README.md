...
[YouTube_Video] << coming soon! (expected 1/17/2026)
...
- The first three prototype parts built measured as **41.76-42.02** x **35.81-36.10** mm
...
**Step 1**. Insert the RAM chip into the base, making sure that the clocking mark is aligned with the matching mark on the tool. The clocking mark on the chip is the smaller circle in the lower left when looking at the chip from the top, with the text right side up.
...
**Step 3**. With the Base and Cover on the table, press down on the Cover onto the Base to bend the pins using mnoderate force, and as evenly as possible. Even if one side bends first, continue pressing until all sides are seated. The tool is designed not to damage the chip or the pins if excessive force is used, nonetheless, once the pins are bent, there is no reason to press any further, or to press any harder.

**Step 4**. Remove the Cover by sliding it upwards and away from the Base.

**Step 5**. The chip will likely be slightly pressed into the Cover (or, less likely, the Base). Use a pen or the eraser end of a pencil through the hole of the Cover (or Base) to pop it out of the tool: make sure to be only an inch (2 cm) above your table surface to prevent any damage to the pins when it falls down. Inspect the chip to make sure that:
...
**Step 1**. Gently align the Chip Alignment Tool with an existing RAM chip on the motherboard. It is important that you do not force it down, but you may feel a very light "click" as it comes down. It is best to lower the tool at an angle to have one outside leg go down first against the motherboard and aligned with the corner pins, then lower the center leg next, and the third outside leg last. Once down, gently confirm that the tool is snug and does not move.
...
**Step 4**. Using two fingers from the same hand, apply light pressure to the alignment tool half-circle and to the top RAM chip, and solder two pins, on the short side, and one on the long side, opposite from each other. *Note: there are multiple soldering techniques of these RAM chips, so checkout videos on youtube, and choose the one that makes sense to you.*

**Step 5**. With one finger pressing lightly on the top chip, gently remove the alignment tool.

**Step 6**. Inspect all four sides to ensure that all pins are still aligned.
